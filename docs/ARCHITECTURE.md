...
`AssetInventory.status(forModules:)` ne dit pas si le modèle est présent sur la machine, mais s'il est attaché au processus qui pose la question. Il rend `.supported` à chaque démarrage et rien ne survit à la sortie du processus. La notion système, elle, est `SpeechTranscriber.installedLocales` : sur cette machine elle contient le français depuis toujours, sans que le statut du module en dise autant.

**Ce qui fait basculer le statut à `.installed`, c'est la réservation, non le téléchargement.** Sonde du 27 juillet 2026 dans un processus neuf, modèle déjà sur le disque : statut `supported`, `reserve(locale:)` rend `true`, statut `installed` immédiatement, `downloadAndInstall()` ensuite ne transfère rien et met 0,4 s. La v2.1.4 attribuait la bascule au téléchargement, ce qui a masqué la cause réelle du bogue pendant deux versions.
...
**Piège n° 3, celui qui a fait rechuter la v2.4.0** : `reserve(locale:)` est déclaré `@discardableResult ... -> Bool`, et ce `Bool` est la seule trace d'un refus. La méthode ne lève rien quand elle ne réserve pas. Le journal montrait donc une réservation apparemment réussie, un téléchargement de sept millisecondes, un statut resté `supported`, et un échec imputé au téléchargement.

Le refus a une cause précise. Quand le système révoque le rattachement sans retirer la locale de `reservedLocales`, `reserve` refuse une réservation qu'il croit déjà faite, et l'app reste bloquée sur une réservation fantôme. `AppleSpeechEngine.attach(_:named:)` traite ce cas en libérant explicitement la locale (`release(reservedLocale:)`) avant de la réserver de nouveau. Le téléchargement n'est plus tenté qu'en dernier recours, et le rattachement rejoué après lui.
