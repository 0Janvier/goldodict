...
public struct TriggerResolver {
    ...
    /// Déclare une dictée démarrée sans passer par le raccourci, depuis le menu.
    ///
    /// Sans cela le résolveur se croirait au repos et le premier appui sur le
    /// raccourci lancerait une seconde dictée par-dessus la première, au lieu de
    /// l'arrêter comme l'utilisateur s'y attend.
    public mutating func adoptToggle() {
        phase = .toggling
    }
}
