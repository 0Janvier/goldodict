#!/bin/bash
# Fabrique Goldodict.app à partir du binaire SwiftPM, le signe avec l'identité
...
cp "$BIN" "$APP/Contents/MacOS/Goldodict"
cp "$ROOT/Resources/Info.plist" "$APP/Contents/Info.plist"
# L'icône est un fichier versionné, pas un produit de build : si elle manque, le
# bundle prend l'icône générique de macOS sans que rien ne le signale.
if [[ -f "$ROOT/Resources/AppIcon.icns" ]]; then
    cp "$ROOT/Resources/AppIcon.icns" "$APP/Contents/Resources/"
else
    echo "  ⚠ Resources/AppIcon.icns absent — lancer scripts/make_icon.sh" >&2
fi
...
if ! security find-identity -v -p codesigning | grep -qF "$SIGN_ID"; then
    ...
    echo "  Surcharger avec GOLDODICT_SIGN_ID=… ou '-' pour une signature ad hoc." >&2
    exit 1
fi
codesign --force --options runtime \
    --entitlements "$ROOT/Resources/Goldodict.entitlements" \
    --sign "$SIGN_ID" "$APP"
codesign --verify --verbose=2 "$APP"

echo "→ installation dans /Applications"
if pgrep -x Goldodict >/dev/null; then
    echo "  (arrêt de l'instance en cours)"
    pkill -x Goldodict || true
    sleep 1
fi
rm -rf "/Applications/Goldodict.app"
cp -R "$APP" "/Applications/Goldodict.app"
